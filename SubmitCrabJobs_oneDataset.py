...
def parseOptions():
    ...
    parser.add_option('-c', '--cfg', dest='CONFIGFILE', type='string', default='/scratch/osg/mhl/CSC_2016/localRecoStudy/CMSSW_6_2_0_SLHC23_patch1/src/UFCSCSoftware/UFCSCRootMaker/test/UFCSCRootMaker_template.py', help='configuration template')
...
def submitAnalyzer():
    ...
    datasetfiles["/SingleMuon/Run2016H-PromptReco-v2/RECO"] = [line.strip() for line in open("fileList_singleMuon2016H.txt", 'r')]
    nfiles["/SingleMuon/Run2016H-PromptReco-v2/RECO"] = len(datasetfiles["/SingleMuon/Run2016H-PromptReco-v2/RECO"])
    print "/SingleMuon/Run2016H-PromptReco-v2/RECO", 'nfiles: ', nfiles['/SingleMuon/Run2016H-PromptReco-v2/RECO']

    datasets = ["/SingleMuon/Run2016H-PromptReco-v2/RECO"]

    for dataset in datasets:

        filename = dataset.split('/')[1]+'_'+dataset.split('/')[2]
        if (len(filename)>99):
          newfilename = filename.split('TuneZ2star')[0]+'PU140'
